...
type Props = { model: string; onModelChange: (m: string) => void; apiKey: string; onApiKeyChange: (k: string) => void; dirty?: boolean }
export function ModelSelector({ model, onModelChange, apiKey, onApiKeyChange, dirty }: Props){
  const [showKey, setShowKey] = useState(false)
  return (
    <div className="card p-4">
      <h3 className="text-lg font-semibold mb-2 flex items-center gap-2">AI Model {dirty && <span title="Model changed. Click 'Analyze' to reprocess with new model." className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-yellow-400 text-black font-bold">!</span>}</h3>
