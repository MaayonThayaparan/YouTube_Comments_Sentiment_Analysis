export function toCSV(rows: any[]) {
  ...
  const headers = ['id','likeCount','totalReplyCount','base','adjusted','textOriginal','publishedAt']
  const escape = (v:any) => { if (v == null) return ''; const s = String(v).replace(/"/g, '""'); return `"${s}"` }
  const lines = [headers.join(',')]
  for (const r of rows) { const vals = [ r.id, r.likeCount, r.totalReplyCount, r.base?.toFixed?.(4) ?? '', r.adjusted?.toFixed?.(4) ?? '', r.textOriginal, r.publishedAt ]; lines.push(vals.map(escape).join(',')) }
  return lines.join('\n')
}
