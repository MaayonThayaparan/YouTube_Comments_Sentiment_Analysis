 ...
 * OpenAI provider
 * ---------------
 * WHAT: Cloud LLM, constrained to return a numeric polarity in [-1,1].
 * WHY: Optional for users who bring an API key and want richer semantics.
 */
...
export class OpenAIProvider extends ISentimentProvider{
  ...
  async analyzeBatch(texts){
    ...
    for(const text of texts){
      // Keep the prompt terse; we parse only the first number.
