 ...
 *       • YouTube comment fetching & sentiment scoring (/api/comments_scored)
 *       • YouTube video metadata enrichment (/api/video_meta)
 *       • Summarization of sentiment themes (/api/summarize)
 *       • Progress polling for long-running jobs (/api/progress)
 *
 * WHY:
 *   - Central backend API for the React client.
 *   - Orchestrates between YouTube Data API and multiple sentiment models:
 *     VADER (local), OpenAI, Gemini, and Ollama LLaMA3.
 *
 * DESIGN:
 *   - Uses in-memory cache + in-memory job progress tracking for simplicity.
 *   - Batch sentiment analysis + summarization to handle API rate limits.
 *   - Pluggable sentiment providers through buildProvider().
 *
 * LIMITATIONS:
 *   - Cache and job progress lost on server restart.
 *   - Not horizontally scalable without external state (e.g. Redis).
 *   - Error handling is intentionally simple: logs + generic responses.
 */
...
/* -------------------------------------------------------------------------- */
/* PROGRESS TRACKING (in-memory, keyed by jobId)                              */
/* -------------------------------------------------------------------------- */
...
if (!API_KEY)
  console.warn("[WARN] YOUTUBE_API_KEY missing — set it in server/.env")
...
/* -------------------------------------------------------------------------- */
/* COMMENTS + SENTIMENT SCORING ENDPOINT                                      */
/* -------------------------------------------------------------------------- */
app.get("/api/comments_scored", async (req, res) => {
  try {
    ...
    // Initialize progress state if jobId provided
    if (jobId)
      progress.set(jobId, {
        totalPages: 0,
        fetchedPages: 0,
    ...
    const items = await fetchAllComments(
      ...
      (p) => {
        if (jobId) {
          ...
          progress.set(jobId, {
            ...cur,
            fetchedPages: p.fetchedPages || cur.fetchedPages,
            totalPages: p.totalPages || cur.totalPages,
          })
        }
      },
    ...
    // Flatten all text (top-level + replies)
    const texts = []
    items.forEach((it) => {
    ...
    for (let i = 0; i < texts.length; i += batchSize) {
      const partial = await provider.analyzeBatch(
      ...
      if (jobId) {
        ...
        progress.set(jobId, {
          ...cur,
          scoredTexts: Math.min(
            texts.length,
            cur.scoredTexts + partial.length
          ),
        })
      }
    }

    // Map scores back to parent + replies
    let k = 0
    const scored = items.map((it) => {
      ...
      return {
        ...it,
        base: Number.isFinite(parentScore) ? parentScore : 0,
        replies,
      }
    })
...
app.post("/api/summarize", async (req, res) => {
  try {
    ...
    async function llamaSumm(textsArr) {
      ...
      for (const c of chunks) {
        ...
        const { data } = await axios.post(
          `${provider.host}/api/generate`,
          { model: provider.model, prompt, stream: false },
          { timeout: 45000 }
        )
      ...
      const { data: final } = await axios.post(
        `${provider.host}/api/generate`,
        { model: provider.model, prompt: finalPrompt, stream: false },
        { timeout: 45000 }
      )
    ...
    if (ctor === "OllamaLlama3Provider")
      return res.json({ summary: await llamaSumm(texts), model })
    if (ctor === "OpenAIProvider")
      return res.json({ summary: await openaiSumm(texts), model })

    // Fallback: simple lexicon-based heuristic
    const sample = texts.slice(0, 500).join(" ")
    const posWords = (
      sample.match(
        /\b(good|great|love|amazing|nice|awesome|excellent|helpful|cool)\b/gi
      ) || []
    ).length
    const negWords = (
      sample.match(
        /\b(bad|terrible|hate|awful|worse|worst|boring|lame|stupid|annoying|fake|trash)\b/gi
      ) || []
    ).length
    ...
    return res.json({
      summary: `• Overall tone is ${tone}.\n• Use OpenAI/Gemini for richer summaries.`,
      model,
    })
  } catch (err) {
...
/* -------------------------------------------------------------------------- */
app.get("/", (_, res) =>
  res.type("text/plain").send("API up. Use the React client on http://localhost:5173")
)
