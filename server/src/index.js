...
app.get("/api/comments_scored",async(req,res)=>{
  try{
    ...
    const items=await fetchAllComments(videoId,API_KEY,200,(p)=>{ if(jobId){ const cur=progress.get(jobId)||{totalPages:0,fetchedPages:0,totalTexts:0,scoredTexts:0}; progress.set(jobId,{...cur,fetchedPages:p.fetchedPages||cur.fetchedPages}) } }, Number(process.env.YT_THROTTLE_MS||200))
...
")}`
    ...
    const posWords=(sample.match(/(good|great|love|amazing|nice|awesome|excellent|helpful|cool)/gi)||[]).length
    const negWords=(sample.match(/(bad|terrible|hate|awful|worse|worst|boring|lame|stupid|annoying|fake|trash)/gi)||[]).length
